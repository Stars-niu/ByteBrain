#pip install streamlit modelscope transformers torch numpy PyMuPDF python-docx
...
# 文件上传功能
uploaded_file = st.file_uploader("上传文件 (PDF或Word):", type=["pdf", "docx"])
...
if uploaded_file:
    ...
    # 将文件内容添加到向量库中
    index.documents.append(file_text)
    index.vectors.append(embed_model.get_embeddings([file_text])[0])
    st.success("文件已成功上传并处理！")
